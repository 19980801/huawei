          ...
          <h6 class="mb-3 p-2 text-muted small">
            <nav aria-label="Page navigation example">
              <ul class="pagination mb-0 justify-content-end">
                <!-- 跟0做比较时需要用全等因为 "" false 都是0 -->
                <li class="page-item" :class="pno==='0'?'disabled':''" @click="subtract"><a class="page-link bg-transparent" href="javascript:;">上一页</a></li>
                <!-- 留一个li模板 --> 
                <!-- v-for可以遍历数字 -->
                <li class="page-item" v-for="i of pageCount" :key="i" :class="i==parseInt(pno)+1?'active':''" @click="loadPage(i)"><a class="page-link" href="javascript:;" v-text="i" :class="i==parseInt(pno)+1?'border':'bg-transparent'" ></a></li>
                ...
                <li class="page-item" :class="pno==pageCount-1?'disabled':''" @click="add"><a class="page-link bg-transparent" href="javascript:;">下一页</a></li>
              </ul>
            </nav>
          </h6>
        </div>
...
<script>
  ...
  // 相当于node中的module.exports={...}
  export default {
    // 必须和router.js中的/:kwords保持一致
    props:["kwords"],
    data(){
      return{
        pno:0,
    ...
    // 每当地址栏中查询关键词发生改变时自动重新请求数据并加载页面
    watch:{
      kwords(){
        // props中kwords变量就会自动获得变化
        // 只要查询条件变化先恢复pno为0
        this.pno=0;
        // 只要kwords变化，就重新请求数据并重新加载页面
        this.load();
      },
      // 利用watch也可以实时监控每个模型变量的变化来调试并发现错误
      pno(){
